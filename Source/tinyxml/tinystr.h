...
class TiXmlString {
	...
	/*	Function to reserve a big amount of data when we know we'll need
	   it. Be aware that this
		function DOES NOT clear the content of the TiXmlString if any
	   exists.
	*/
	...
	void init(size_type sz, size_type cap) {
		if (cap) {
			...
			//	rep_ = static_cast<Rep*>(operator
			// new(sizeof(Rep) + cap));
			// doesn't work in some cases of new being overloaded.
			// Switching
			// to the normal allocation, although use an 'int' for
			// systems
	...
	void quit() {
		if (rep_ != &nullrep_) {
			// The rep_ is really an array of ints. (see the
			// allocator, above).
			// Cast it back before delete, so the compiler won't
			// incorrectly call destructors.
