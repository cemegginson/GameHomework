...
void TiXmlBase::EncodeString(const TIXML_STRING& str, TIXML_STRING* outString) {
	...
	while (i < (int)str.length()) {
		...
		if (c == '&' && i < ((int)str.length() - 2) &&
			...
			TIXML_SNPRINTF(buf, sizeof(buf), "&#x%02X;",
				       (unsigned)(c & 0xff));
#else
			...
			*outString +=
			    (char)c; // somewhat more efficient function call.
			++i;
		}
	}
}
	...
	if (StringEqual(node->Value(), "true", true, TIXML_ENCODING_UNKNOWN) ||
		...
		*bval = true;
		result = TIXML_SUCCESS;
			       ...
			       TIXML_ENCODING_UNKNOWN) ||
		   StringEqual(node->Value(), "no", true,
			       TIXML_ENCODING_UNKNOWN) ||
		   StringEqual(node->Value(), "0", true,
			       TIXML_ENCODING_UNKNOWN)) {
		*bval = false;
		result = TIXML_SUCCESS;
	}
...
bool TiXmlDocument::LoadFile(FILE* file, TiXmlEncoding encoding) {
	...
	// Get the file size, so we can pre-allocate the string. HUGE speed
	// impact.
	long length = 0;
	...
	// in external
	// parsed entities (including the document entity) on input, before
	// parsing, by translating
	// both the two-character sequence #xD #xA and any #xD that is not
	// followed by #xA to
	// a single #xA character.
	// </quote>
	//
	// It is not clear fgets does that, and certainly isn't clear it works
	// cross platform.
	// Generally, you expect fgets to translate from the convention of the
	// OS to the c/unix
	...
	*/
	...
	// Process the buffer in place to normalize new lines. (See comment
	// above.)
	...
	// CR (Carriage return, '\r', 0x0D, 13 in decimal) individually, or CR
	// followed by LF (CR+LF, 0x0D 0x0A)...
	//		* LF:    Multics, Unix and Unix-like systems (GNU/Linux,
	// AIX, Xenix, Mac OS X, FreeBSD, etc.), BeOS, Amiga, RISC OS, and
	// others
	//		* CR+LF: DEC RT-11 and most other early non-Unix,
	//non-IBM
	// OSes, CP/M, MP/M, DOS, OS/2, Microsoft Windows, Symbian OS
	//		* CR:    Commodore 8-bit machines, Apple II family, Mac
	//OS
