...
class TiXmlBase {
	...
	*/
	...
	/**	The world does not agree on whether white space should be kept
	   or
		not. In order to make everyone happy, these global, static
	   functions
	...
	*/
	...
	/** Return the position, in the original source file, of this node or
	   ...
	   column is
		1,1). If the returns values are 0 or less, then the parser does
	   not have
	   ...
	   programmatically
		(by adding or changing nodes and attributes) the new values will
	   NOT update to
		reflect changes in the document.

		There is a minor performance cost to computing the row and
	   column. Computation
		can be disabled if TiXmlDocument::SetTabSize() is called with 0
	   as the value.
	...
	*/
	...
	inline static bool IsWhiteSpace(int c) {
		...
		return false; // Again, only truly correct for
			      // English/Latin...but usually works.
	}

#ifdef TIXML_USE_STL
	static bool StreamWhiteSpace(std::istream* in, TIXML_STRING* tag);
	static bool StreamTo(std::istream* in, int character,
			     TIXML_STRING* tag);
#endif
	...
	*/
	...
	/*	Reads text. Returns a pointer past the given end tag.
		Wickedly complex options, but it keeps the (sensitive) code in
	   one place.
	*/
	static const char*
	...
	inline static const char* GetChar(const char* p, char* _value,
		...
		if (*length == 1) {
			...
			*_value = *p;
			...
			// strncpy( _value, p, *length );	// lots of
			// compilers don't like this function (unsafe),
	...
	// Return true if the next characters in the stream are any of the
	// endTag sequences.
	...
	static bool StringEqual(const char* p, const char* endTag,
				bool ignoreCase, TiXmlEncoding encoding);
...
class TiXmlNode : public TiXmlBase {
	...
	*/
	...
	/** An output stream operator, for every class. Note that this outputs
		...
		The operator<< and operator>> are not completely symmetric.
	   Writing
		a node to a stream is very well defined. You'll get a nice
	   stream
		of output, without any extra whitespace or newlines.

		But reading is not as well defined. (As it always is.) If you
	   create
	...
	*/
	...
	const TiXmlNode* FirstChild() const {
		return firstChild;
	} ///< The first child of this node. Will be null if there are no
	/// children.
	TiXmlNode* FirstChild() { return firstChild; }
	const TiXmlNode* FirstChild(const char* value) const; ///< The first
	/// child of this
	/// node with the
	/// matching
	///'value'. Will
	/// be null if none
	/// found.
	/// The first child of this node with the matching 'value'. Will be null
	/// if none found.
	...
	const TiXmlNode* LastChild() const {
		return lastChild;
	} /// The last child of this node. Will be null if there are no
	  /// children.
	TiXmlNode* LastChild() { return lastChild; }

	const TiXmlNode* LastChild(const char* value) const; /// The last child
							     /// of this node
							     /// matching
							     /// 'value'. Will
							     /// be null if
	...
	*/
	...
	/// This flavor of IterateChildren searches for children with a
	/// particular 'value'
	...
	*/
	...
	/** Add a new node related to this. Adds a child past the LastChild.
		...
		henceforth owned (and deleted) by tinyXml. This method is
	   efficient
	...
	*/
	...
	/** Add a new node related to this. Adds a child before the specified
	   child.
		Returns a pointer to the new object or NULL if an error occured.
	*/
	...
	/** Add a new node related to this. Adds a child after the specified
	   child.
		Returns a pointer to the new object or NULL if an error occured.
	*/
	...
	*/
	...
	virtual const TiXmlDocument* ToDocument() const {
		return 0;
	} ///< Cast to a more defined type. Will return null if not of the
	/// requested type.
	...
	*/
	...
	/** Accept a hierchical visit the nodes in the TinyXML DOM. Every node
	   in the
		XML tree will be conditionally visited and the host will be
	   called back
		via the TiXmlVisitor interface.

		This is essentially a SAX interface for TinyXML. (Note however
	   it doesn't re-parse
...
class TiXmlAttribute : public TiXmlBase {
...
#endif
	int IntValue() const; ///< Return the value of this attribute, converted
	/// to an integer.
	double DoubleValue() const; ///< Return the value of this attribute,
	...
	/** QueryIntValue examines the value string. It is an alternative to the
		...
		A specialized but useful call. Note that for success it returns
	   0,
...
class TiXmlAttributeSet {
	...
	TiXmlAttribute* FindOrCreate(const std::string& _name);
      ...
      private:
	//*ME:	Because of hidden/disabled copy-construktor in
	// TiXmlAttribute (sentinel-element),
	//*ME:	this class must be also use a hidden/disabled
	// copy-constructor !!!
...
class TiXmlElement : public TiXmlNode {
	...
	*/
	...
	/** QueryIntAttribute examines the attribute - it is an alternative to
	   the
...
#ifdef TIXML_USE_STL
	/// QueryStringAttribute examines the attribute - see
	/// QueryIntAttribute().
	...
	*/
	...
	/** Convenience function for easy access to the text inside an element.
	   ...
	   returned.
		...
		This is a convenient method for getting the text of simple
	   contained text:
		@verbatim
		...
		Note that this function can be misleading. If the element foo
	   was created from
		this XML:
		@verbatim
		...
		then the value of str would be null. The first child node isn't
	   a text node, it is
	...
	*/
	...
	virtual const TiXmlElement* ToElement() const {
		return this;
	} ///< Cast to a more defined type. Will return null not of the
	/// requested type.
	virtual TiXmlElement* ToElement() {
		return this;
	} ///< Cast to a more defined type. Will return null not of the
...
class TiXmlDocument : public TiXmlNode {
	...
	*/
	...
	/** Load a file using the given FILE*. Returns true if successful. Note
	   that this method
		doesn't stream - the entire object pointed at by the FILE*
		will be interpreted as an XML file. TinyXML doesn't stream in
	   XML from the current
	...
	*/
	...
	/** Parse the given null terminated block of xml data. Passing in an
	   encoding to this
		method (either TIXML_ENCODING_LEGACY or TIXML_ENCODING_UTF8 will
	   force TinyXml
		to use that encoding, regardless of what TinyXml might otherwise
	   try to detect.
	*/
	virtual const char*
	Parse(const char* p, TiXmlParsingData* data = 0,
	      TiXmlEncoding encoding = TIXML_DEFAULT_ENCODING);

	/** Get the root element -- the only top level element -- of the
	   document.
		In well formed XML, there should only be one. TinyXml is
	   tolerant of
	...
	*/
	...
	/** Generally, you probably want the error string ( ErrorDesc() ). But
	   if you
	...
	*/
	...
	/** Returns the location (if known) of the error. The first column is
	   column 1,
		and the first row is row 1. A value of 0 means the row and
	   column wasn't applicable
		(memory errors, for example, have no row/column) or the parser
	   lost the error. (An
	...
	*/
	...
	/** SetTabSize() allows the error reporting functions (ErrorRow() and
	   ...
	   stored
		when the file is loaded. Very useful for tracking the DOM back
	   in to
		the source file.
	   ...
	   operator>>.
		...
		The tab size needs to be enabled before the parse or load.
	   Correct usage:
		@verbatim
		TiXmlDocument doc;
		doc.SetTabSize( 8 );
	...
	*/
	...
	/** If you have handled the error, it can be reset with this call. The
	   error
	...
	*/
	...
	/* Write the document to a string using formatted printing ("pretty
	   ...
	   pointer. The
		calling code pust call delete[] on the return char* to avoid a
	   memory leak.
	*/
	...
	*/
	...
	bool useMicrosoftBOM; // the UTF-8 BOM were found when read. Note this,
			      // and try to write.
};
...
class TiXmlPrinter : public TiXmlVisitor {
	...
	*/
	...
	/** Switch over to "stream printing" which is the most dense formatting
	   without
		linebreaks. Common when the XML is needed for network
	   transmission.
	*/
